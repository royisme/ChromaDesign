...
const mix = (color: [number, number, number], target: [number, number, number], weight: number): [number, number, number] => {
  return [
...
export const getWcagRating = (ratio: number): { score: string; label: string; pass: boolean } => {
